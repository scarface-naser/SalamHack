...
$target_dir = "resources/categoryPictures/";
...
if (isset($_POST['AddCategory'])) {
  if (empty($post01) || empty($post02)) {
    $Error = '<p class="style13">Please fill in all data</p>';
  } else {
    if ($imageFileType2 != "" && $imageFileType2 != "jpg" && $imageFileType2 != "png" && $imageFileType2 != "jpeg" && $imageFileType2 != "gif" && $imageFileType2 != "pdf" && $imageFileType2 != "webp") {
    ...
    if ($uploadOk == 1) {
      move_uploaded_file($u_img_tmp2, "resources/categoryPictures/$newimgproblem2");
      ...
      if ($_FILES["show_img2"]["size"] > 500000) {
        ...
        if ($imageFileType2 == "png") {
          ...
          @aborahaf('resources/categoryPictures/' . $newimgproblem2, 0.5);
        }
        # IF Image gif type.
    ...
    if (mysqli_query($home, $naser)) {
      echo '
                      ...
                      <div class="session-message">
                          <p>Thank you, the Category has been added successfully</p>
                      </div>
...
<head>
  ...
  <link rel="stylesheet" href="style.css"> <!-- Your general CSS file -->
...
<body>
    ...
    <div class="useraddtool-card">
      <form id="useraddtool-form" action="" method="post" enctype="multipart/form-data">
        ...
        <div class="useraddtool-input-group">
          ...
          <input type="text" name="get01" id="useraddtool-category" placeholder="Category Name" required>
        </div>

        <div class="useraddtool-input-group">
          <i class="fa-solid fa-audio-description"></i>
          <textarea id="useraddtool-tool-description" name="get02" placeholder="Simple Description" required></textarea>
        </div>
        <input name="AddCategory" class="useraddtool-add-btn" type="submit" value="Add Category" />
      </form>
    </div>
  </div>
  <script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const uploadBox = dropArea.querySelector(".useraddtool-upload-box"); // Get the upload box
    ...
    fileInput.addEventListener("change", function() {
      if (fileInput.files.length > 0) {
        ...
        reader.onload = function(event) {
          ...
          // Create and append new image inside the upload box
          const img = document.createElement("img");
          img.src = event.target.result;
          img.classList.add("useraddtool-image-preview"); // Add a class for styling
          img.style.width = "100%";
          img.style.height = "100%";
          img.style.objectFit = "cover"; // Make it fit properly
